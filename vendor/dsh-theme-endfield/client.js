...
function apply(ctx) {
    ...
    const isHeroVisible = () => findVisibleHeadline() !== null
    ...
    disposeStyles = insertCss(`
      ...
      /* 对话区玻璃：只落在「消息列表区域」（scrollBody 内的 viewArea，即
         AI 回复消息显示的区域），不碰会话标题栏、也不碰输入框（composerSeat，
         官方输入框是「透明文字 + 字形背衬层」设计，任何背景/blur 都会让
         文字黑掉/模糊，用户已多次反馈）。blur 取低值：用户要求「模糊低
         一点、稍微能看清后面的花纹」——4px 让花纹以柔化轮廓隐约透出，
         文字仍在其上层完全清晰。 */
