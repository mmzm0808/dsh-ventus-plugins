...
function apply(ctx) {
    ...
    const mountPointFor = () => {
      ...
      if (conv !== null) return { mode: 'persist', parent: nonScrollingAncestor(conv) }
    ...
    disposeStyles = insertCss(`
      ...
      /* 对话区玻璃：覆盖整个对话列（含标题栏），消除消息区与标题栏之间
         的缝隙（用户反馈「覆盖不全」）。blur 取低值（4px）让花纹柔化透出、
         文字仍清晰。输入框（composerSeat）豁免 blur——官方输入框是「透明
         文字 + 字形背衬层」设计，任何 blur 都会让文字黑掉（用户多次反馈）；
         只去掉 blur 与自身背景，玻璃底色（root 的）仍在其下层透出。 */
      body.theme-endfield-surface-glass [class$='_centerCol'] [class*='_root'] {
      ...
      body:has([role='dialog'][aria-modal='true'], [data-dsh-sidebar-window]) [class$='_centerCol'] [class*='_root'] {
        backdrop-filter: none !important;
        -webkit-backdrop-filter: none !important;
      }
