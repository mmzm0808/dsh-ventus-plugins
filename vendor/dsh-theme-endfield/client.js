...
function insertCss(css) {
  ...
  // setAttribute 而非 dataset：进程内测试桩的元素没有实现 dataset。
  el.setAttribute('data-endfield-css', '')
...
function apply(ctx) {
    ...
    const syncSurface = () => {
      ...
      const on = isEnabled()
      // add/remove 而非 classList.toggle：进程内测试桩的 classList 只有 add/remove。
      const setClass = (name, present) => {
        if (present) document.body.classList.add(name)
        else document.body.classList.remove(name)
      }
      setClass(SURFACE_CLASS_GLASS, on && surface === 'glass')
      setClass(SURFACE_CLASS_SOLID, on && surface === 'solid')
      setClass(SURFACE_CLASS_TRANSPARENT, on && surface === 'transparent')
      setClass(SURFACE_CLASS_CONV, on && surface === 'solid' && isSurfaceConvOn())
      // 进程内测试桩的 style 没有 setProperty，缺 API 时跳过自定义色注入。
      if (document.body.style !== undefined && typeof document.body.style.setProperty === 'function') {
    ...
    /* The conversation column (frame children: sidebar / CENTER / details).
       水印的挂载点与居中基准仍以此列为准（见 mountPointFor / positionWatermark）；
       等高线层本身已改为 frame 级整铺（上游架构），不再按此列定位。 */
    const findCenterCol = () => {
    ...
    disposeStyles = insertCss(`
      ...
      /* 玻璃副作用修正：viewArea 的 backdrop-filter 会让它成为层叠上下文，
         把里面的 hero/模型下拉框困在「level 0 / DOM 序」——与 body 级
         z-index:0 的 hero 水印平局，水印 DOM 靠后而压到下拉框上（实测
         11838px 被盖住）。给 viewArea 显式 relative+z1，让对话区内容稳定
         压过水印。relative 不捕获 fixed 子级（backdrop-filter 早已捕获），
         不改变应用布局。 */
      body.theme-endfield-surface-glass [class$='_centerCol'] [class$='_viewArea'] {
      ...
      /* persist 水印锚定：对话列成为定位祖先（absolute inset0 的水印精确
         ...
         关键修正（对齐上游）：用 :has(> 水印) 门控，只在列真挂 persist 水印时
         才建层叠上下文——无条件常建会把 hero 的 z-index:1 锁死在列内，让 body 级
         z-index:0 的 hero 水印靠 DOM 后序赢下平局、压到模型下拉框上（实测）。 */
    ...
    const LOCALE_ZH = {
      ...
      surfaceRow: '侧边栏表面',
      surfaceGlass: '玻璃',
      surfaceTransparent: '透明',
      surfaceSolid: '自定义',
      surfaceHintGlass: '半透明基底 + 背景模糊，等高线花纹柔化透出（窄栏与宽栏同效）',
      surfaceHintTransparent: '侧边栏完全透明，等高线花纹直接贯穿',
      surfaceHintSolid: '使用右侧调色盘选定的不透明纯色底色',
      surfaceColorLabel: '侧边栏底色',
      surfaceConvRow: '应用到对话区',
      surfaceConvOn: '已开启',
      surfaceConvOff: '已关闭',
      surfaceConvHint: '对话区使用同一底色，但比侧边栏多一层半透明（花纹仍可透过）',
      loaderRow: '启动加载动画',
      loaderOn: '开启动画',
      loaderOff: '关闭动画',
    ...
    const LOCALE_EN = {
      ...
      surfaceRow: 'Sidebar surface',
      surfaceGlass: 'Glass',
      surfaceTransparent: 'Transparent',
      surfaceSolid: 'Custom',
      surfaceHintGlass: 'Translucent base + backdrop blur; the contour sheet shows through, softened',
      surfaceHintTransparent: 'Fully transparent sidebar; the contour sheet runs straight through',
      surfaceHintSolid: 'Opaque custom colour picked from the swatch on the right',
      surfaceColorLabel: 'Sidebar fill',
      surfaceConvRow: 'Apply to conversation',
      surfaceConvOn: 'On',
      surfaceConvOff: 'Off',
      surfaceConvHint: 'The conversation column takes the same colour with extra translucency (the pattern still shows through)',
        ...
        () => {
          ...
          return R.createElement('div', { style: pageStyle }, [
            ...
            R.createElement('div', { key: 'group-bg' }, [
              ...
              /* 侧边栏表面（本地特性）：玻璃 / 透明 / 自定义纯色 + 调色盘。 */
              row('surface', surface !== 'solid', [
                R.createElement('span', { style: labelStyle },
                  t('surfaceRow') + t('sep') + t(surface === 'glass' ? 'surfaceGlass' : surface === 'solid' ? 'surfaceSolid' : 'surfaceTransparent'),
                  R.createElement('span', { style: hintStyle },
                    t(surface === 'glass' ? 'surfaceHintGlass' : surface === 'solid' ? 'surfaceHintSolid' : 'surfaceHintTransparent')
                  )
                ),
                R.createElement('span', { style: { display: 'flex', gap: '8px', flex: '0 0 auto', alignItems: 'center' } },
                  R.createElement('button', {
                  ...
                  surface === 'solid'
                    ? R.createElement('input', {
                        ...
                        'aria-label': t('surfaceColorLabel'),
                        style: {
              ...
              surface === 'solid'
                ? row('surface-conv', true, [
                  R.createElement('span', { style: labelStyle },
                    t('surfaceConvRow'),
                    R.createElement('span', { style: hintStyle }, t('surfaceConvHint'))
                  ),
                  R.createElement('button', {
                    type: 'button',
                    onClick: toggleSurfaceConv,
                    style: btnStyleFor(surfaceConv),
                  }, t(surfaceConv ? 'surfaceConvOn' : 'surfaceConvOff'))
                ])
                : null,
            ]),
            /* --- 03 动画：启动加载动画 --- */
