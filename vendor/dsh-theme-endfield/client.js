...
function apply(ctx) {
    ...
    /* 对话区横向滚动归零：PgUp/PgDn 在输入框有文字时会带动内层滚动
       容器的 scrollLeft（整体左移，每次约一个细栏宽）。CSS 禁横向会
       裁剪浮窗/工具栏/上拉框，改用 capture 滚动监听精准归零——排除
       输入框、代码块与浮窗容器（它们需要原生横向滚动）。 */
    if (typeof document !== 'undefined' && typeof window !== 'undefined') {
      document.addEventListener('scroll', (e) => {
        const t = e.target
        if (!(t instanceof HTMLElement)) return
        if (t.scrollLeft === 0) return
        const cls = typeof t.className === 'string' ? t.className : ''
        if (t.tagName === 'TEXTAREA' || t.tagName === 'PRE' || t.tagName === 'CODE') return
    ...
    /* ---------- sidebar surface mode (settings dropdown) ----------
       The sidebar root paints its own fill (--dsw-specific-sidebar-fill, which
       this theme sets equal to bg-base). Surface mode restyles it:
         glass       translucent base + backdrop blur — the contour sheet bleeds
                     through softly instead of being hidden;
         transparent the sheet runs through the sidebar unbroken;
         solid       an opaque custom colour from the settings palette.
       Only the SIDEBAR is touched; the centred ENDFIELD sheet and the
    ...
    const readSurfaceColor = () => {
      ...
      return typeof value === 'string' && /^#[0-9a-fA-F]{6}$/.test(value) ? value : '#101110'
    }
    const isSurfaceConvOn = () => (typeof localStorage !== 'undefined' && localStorage.getItem(SURFACE_CONV_KEY)) !== '0'
    const syncSurface = () => {
      if (typeof document === 'undefined' || document.body === null) return
    ...
    const isHeroVisible = () => {
      if (findVisibleHeadline() === null) return false
      /* 有消息流的会话页不是 hero：消息区（data-chat-flow，仅在有消息的
         会话页存在）在屏时，水印必须走 persist 模式贴在对话区背景层随
         对话区布局，而不是 fixed 浮在视口中部（实测用户反馈：有历史
         对话页的 ENDFIELD 字母位置不对——正是 hero 模式误判导致）。 */
      if (typeof document !== 'undefined' && document.querySelector('[data-chat-flow]') !== null) return false
      return true
    }
    /** The visible conversation column — the persist-mode anchor and mount parent. */
    const findConversationRoot = () => {
      if (typeof document === 'undefined') return null
      // The conversation column root (ConversationRoot) carries a stable
      // `data-phase` attribute AND a CSS-module class — '<hash>_root' on the
      // build this theme was authored against, '_root_<hash>' on Vite builds.
      // Prefer the data-attribute match (hash-independent, exact); fall back
      // to the largest visible flex-column root (a fixed overlay/panel roots
      // are rows or narrower) only when the attribute is absent.
      const anchored = document.querySelector('[data-phase][class*="wSkVaW_root"], [data-phase][class*="_root_"]')
      let best = anchored
      if (best === null) {
        const all = document.querySelectorAll('[class*="wSkVaW_root"], [class*="_root_"]')
        let bestArea = 0
        for (const el of all) {
          const r = el.getBoundingClientRect()
          if (r.width > 0 && r.height > 0) {
            const cs = typeof getComputedStyle === 'function' ? getComputedStyle(el) : null
            if (cs !== null && (cs.display !== 'flex' || cs.flexDirection !== 'column')) continue
      ...
      if (best !== null) best.setAttribute('data-endfield-conv', '')
      return best
    }
    ...
    /* The persist mark must never ride the conversation column's scroll: an
       absolute child of the scrolling box follows its content (measured
       y=-4276 after scrolling), so the wordmark lands far off-viewport or
       appears in odd gaps. Climb to the first non-scrolling ancestor — the
       column's outer container or higher — and mount there instead; the
       column keeps its own islate/relative scaffold from the stylesheet. */
    const nonScrollingAncestor = (el) => {
      let node = el
    ...
    const mountPointFor = () => {
      ...
      if (conv !== null) {
        /* persist 直接锚定对话列（centerCol）：nonScrollingAncestor 在某些
           布局会爬到页面级容器，水印以页面宽度居中（实测桌面端用户反馈：
           有历史会话的大白字以整窗居中而非会话区宽度）。centerCol 本身
           overflow:hidden 不滚动（滚动区在其内部），配合样式表给 centerCol
           的 position:relative + isolation:isolate，absolute inset0 的水印
           精确居中于对话区宽。 */
        const col = findCenterCol()
        if (col !== null) return { mode: 'persist', parent: col }
        return { mode: 'persist', parent: nonScrollingAncestor(conv) }
      }
    ...
    /* ---------- contour (topographic) background ------------------------------
       ...
       WHERE IT IS MOUNTED. The layer is a fixed-position <body> child sized and
       positioned to the CONVERSATION column (frame child 2), not the frame: the
       sheet must stay inside the conversation width, centred on it, so an opened
       details column on the right never covers it. Measured from the app's own
       CSS, the frame and the column roots paint an OPAQUE --dsw-alias-bg-base
       over any body-level layer; those fills are neutralised to transparent while
       the layer is mounted (the :has() guard makes all of it vanish when off),
       and the body keeps its own base fill as the backing colour. The sidebar
       keeps its own colour because in this theme
       --dsw-specific-sidebar-fill and --dsw-alias-bg-base are the same value, so
       making it transparent changes no pixel except letting the sheet through.
    ...
    let contourBlurCv = null    // 侧边栏区域的花纹模糊层（毛玻璃晕染，不依赖 backdrop-filter）
    let contourRaf = null
    ...
    const contourDrawLines = () => {
      ...
      ctx.stroke()
      /* 侧边栏区域模糊副本：从清晰层裁剪左侧栏宽的部分，blur 后画到
         模糊层——毛玻璃晕染不依赖 backdrop-filter。 */
      if (contourBlurCv !== null) {
        const bctx = contourBlurCv.getContext('2d')
        if (bctx) {
    ...
    const findAppFrame = () => {
      if (typeof document === 'undefined') return null
      // Primary: the app-frame shell root identified by its stable
      // `[data-shell-overlay]` child (rc2 layout, unique to the frame).
      // `_centerCol` suffix matching proved unstable across boot stages
      // (measured: it can resolve to a zero-width column whose parent is
      // NOT the frame, mounting the contour layer into a 56px sliver that
      // then hides the conversation column).
      const root = document.querySelector('div:has(> [data-shell-overlay])')
      if (root !== null) {
        const rr = root.getBoundingClientRect()
        if (rr.width > 0 && rr.height > 0) return root
      }
    ...
    /* The conversation column (frame children: sidebar / CENTER / details).
       The contour sheet is sized and centred on THIS column, not the frame:
       the sheet must live inside the conversation width so an opened details
       column on the right never covers it. Follows the column live via
       ResizeObserver (sidebar collapse/expand and details open/close all
       change its rect). */
    const findCenterCol = () => {
      const frame = findAppFrame()
      if (frame === null) return null
      const col = frame.children[1]
    ...
    const contourSizeTo = (host) => {
      ...
      // 模糊层：覆盖侧边栏宽（默认 280，跟随悬浮侧边栏宽度变量）。
      if (contourBlurCv !== null) {
        const bw = Math.max(56, Math.round(parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--dsh-float-sidebar-w')) || 280))
        contourBlurCv.width = Math.min(bw, w)
        contourBlurCv.height = h
        contourBlurCv.style.width = contourBlurCv.width + 'px'
        contourBlurCv.style.height = h + 'px'
      }
      // Also size the wrapper itself: inset:0 normally extends it to the frame,
      // but measured environments (grid-participating frames / rule races) can
      // resolve the layer to a 56px sliver that blanks the conversation column.
      if (contourWrap !== null) {
        contourWrap.style.width = w + 'px'
        contourWrap.style.height = h + 'px'
      }
      return true
    }
    ...
    const syncContour = () => {
      ...
      /* Fast path for the ALREADY-MOUNTED case. This runs from a subtree
         ...
         findCenterCol() here is what avoids forcing a synchronous reflow on
         every mutation. Note it skips only the mount work — the switch
         reconciliation below still runs, because that is how a settings
         toggle takes effect while the layer is already on screen. */
      const attached = contourWrap !== null && contourHost !== null
        && contourWrap.parentNode === document.body
        && contourWrap.isConnected && contourHost.isConnected
      if (!attached) {
        const host = findCenterCol()
        if (host === null) {
        ...
        if (contourWrap === null) {
          ...
          /* Inline positioning + body-level mount: the stylesheet rule exists,
             but inside the grid-participating frame the layer can resolve to a
             56px sliver (measured, with explicit width:1508px even!) that
             blanks the conversation column. Mounting at the body root with
             inline absolute/inset:0 makes it a true full-viewport underlay
             (same pattern as the watermark), and the canvas is sized from the
             frame's box in contourSizeTo(). */
          wrap.style.cssText = 'position:absolute;left:0;top:0;width:0;height:0;z-index:0;pointer-events:none;overflow:hidden;'
          const line = document.createElement('canvas')
          line.setAttribute('data-endfield-contour-lines', '')
          line.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;'
          wrap.appendChild(line)
          contourLineCv = line
          /* 侧边栏区域模糊层：覆盖左侧栏宽（默认 280px）的花纹模糊副本，
             玻璃模式下显示——毛玻璃晕染不依赖 backdrop-filter（父级有
             transform/filter 时 backdrop-filter 采样失效，实测不生效）。 */
          const blur = document.createElement('canvas')
          blur.setAttribute('data-endfield-contour-blur', '')
          blur.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;'
          wrap.appendChild(blur)
          contourBlurCv = blur
          // Body root, first child → paints below the app (later siblings win).
    ...
    /* 侧边栏折叠/展开只翻转 frame 的 data-sidebar-collapsed 属性，center col
       ...
       重挂（重新测量 + 重新观察），否则仅重算尺寸/位置。better-sidebar 把
       同一属性挂在 body 上，悬浮侧边栏挂在 frame 上——观察 documentElement
       子树属性，两者都覆盖。 */
    let contourBsbObserver = null
    if (typeof MutationObserver !== 'undefined' && typeof document !== 'undefined' && document.documentElement) {
      contourBsbObserver = new MutationObserver(() => {
        if (contourWrap === null) return
        const host = findCenterCol()
        if (host === null) return
        if (host !== contourHost) {
          // center col 被 React 重建：旧 host 与旧 RO 都已失效，重挂。
          contourTeardown()
          syncContour()
          return
        }
        if (contourSizeTo(host)) {
          contourExtract(contourPhase)
          contourDrawLines()
        }
      })
      contourBsbObserver.observe(document.documentElement, { attributes: true, subtree: true, attributeFilter: ['data-sidebar-collapsed'] })
    }
    ...
    disposeStyles = insertCss(`
      ...
      /* The persist-mode watermark is a z-index:-1 child of the conversation column.
         ...
         The [data-endfield-conv] attribute (set by findConversationRoot) keeps the
         rule hash-independent: the column's CSS-module class differs across DSH
         builds ('<hash>_root' vs Vite's '_root_<hash>'). */
      [data-endfield-conv]:has(> [data-endfield-watermark]) {
        isolation: isolate;
        position: relative;
      }
      ...
      /* 对话区滚动容器禁止横向滚动：内容横向溢出时 PgUp/PgDn 的
         ...
         精准归零（见 syncContour 附近）。 */
      [class$='_scrollBody'] {
        overflow-x: hidden !important;
      }
      /* 输入框（composer textarea）保持原生横向滚动（长行内容、
         光标可见性），不禁用——禁用会导致编辑键（方向/PgUp/PgDn/
         Home/End）光标移动不可见。 */
      [class$='_input'],
      textarea {
        overflow-x: auto;
      ...
      /* 侧边栏区域模糊层：只在玻璃表面模式下显示（毛玻璃晕染）。 */
      [data-endfield-contour] [data-endfield-contour-blur] {
        display: none;
      }
      body.theme-endfield-surface-glass [data-endfield-contour] [data-endfield-contour-blur] {
        display: block;
      }
      /* Why these transparency rules exist. Measured from the app's own
         stylesheets: the frame, the sidebar root and the conversation column
         each paint an OPAQUE var(--dsw-alias-bg-base). Any of them left opaque
         hides the sheet completely on a real page, which is exactly why the
         existing watermark has to mount INSIDE the conversation column instead.
         The layer mounts at the BODY root (see syncContour), so the frame must
         stop painting its own background; the body keeps its base fill as the
         backing colour underneath the sheet, so the composited result is
         unchanged except that the contour is now visible through it.

         Hash notes: rc2 class lists are multi-token like
         '<hash>_root <hash>_collapsed …', so a [class$='_root'] suffix match
         (end of the WHOLE class attribute) never hits; [class*='_root']
         matches any token containing _root, covering both '<hash>_root' and
         Vite's '_root_<hash>' forms. */
      body:has([data-endfield-contour]) div:has(> [data-shell-overlay]) {
        background: transparent !important;
      }
      body:has([data-endfield-contour]) [class*='wSkVaW_root'],
      body:has([data-endfield-contour]) [class*='ydkMvW_root'],
      body:has([data-endfield-contour]) [class*='_root'] {
        background: transparent !important;
      }
      ...
      body:has([data-endfield-contour]) [class$='_sidebarCol'] {
        background: transparent !important;
      }
      ...
      body:has([data-endfield-contour]) [class$='_composerSeat'] {
        background: linear-gradient(180deg,
          rgba(0, 0, 0, 0) 0px,
          color-mix(in srgb, var(--dsw-alias-bg-base) 82%, transparent) 36px) !important;
      }
      /* ---------- sidebar surface mode ----------
         Glass: translucent base + backdrop blur over the contour sheet.
         The base is deliberately lighter than a "solid" fill — the blur is
         what carries the material, the colour only needs to mute what is
         behind (a 68% base reads as plain black; 45% keeps the sheet visible
         as a soft haze). The conversation column gets the SAME treatment so
         text over the sheet stays readable. Solid: the custom colour from the
         settings palette. These selectors carry one extra class + the column
      ...
      /* better-sidebar 右侧面板玻璃：与左侧栏一致的毛玻璃质感。
         ...
         玻璃才透得出背后内容。 */
      body.theme-endfield-surface-glass [data-dsh-panel-host] [class*="panel"],
      body.theme-endfield-surface-glass [data-dsh-panel-host] [class*="bottomPanel"] {
        background:
          linear-gradient(180deg, rgba(255, 255, 255, 0.08), rgba(255, 255, 255, 0.02) 30%, transparent 55%),
          linear-gradient(90deg, rgba(255, 255, 255, 0.04), transparent 25%),
          color-mix(in srgb, #14171c 22%, transparent) !important;
      ...
      /* 对话区玻璃：覆盖「消息列表区（viewArea）+ 会话标题栏（_header）」，
         消除两者之间的缝隙（用户反馈「覆盖不全」）。blur 取低值（4px）让
         花纹柔化透出、文字仍清晰。不覆盖整个 centerCol root——root 会连带
         覆盖上下文窗口/权限上拉框等浮层，半透明深色透出显得发黑（实测
      ...
      body.theme-endfield-surface-glass [class$='_centerCol'] [data-composer-seat],
      body.theme-endfield-surface-glass [class$='_centerCol'] [data-composer-seat] * {
      ...
      /* 纯色表面「应用到对话区」子开关：对话区整体（含标题栏）使用自定义
         纯色的半透明版本（68%）——比侧边栏实底多一层透明，等高线花纹仍
         可透过，文字保持清晰；无 blur，输入框背衬层不受影响。仅当
         surface=solid 且子开关打开时 body 才带 theme-endfield-surface-conv。 */
      body.theme-endfield-surface-solid.theme-endfield-surface-conv [class$='_centerCol'] [class*='_root'] {
        background: color-mix(in srgb, var(--edge-sidebar-solid, #101110) 68%, transparent) !important;
      }
      /* 官方侧边栏按钮（设置/插件市场/工作台等）交互高亮：中性底色。
         主题的 info/accent 反色 token 会让部分按钮 hover 变青或丢失反馈
         （桌面端实测）；统一为官方默认 hover 底色，任何按钮悬停都有清晰
         高亮。 */
      [class$='_sidebarCol'] button:hover {
        background: var(--dsw-alias-interactive-bg-hover) !important;
      }
      /* 会话项三点菜单按钮：任何表面模式下背景必须全透明，只显示三个点。
         主题的表面/按钮 token 覆盖会把它染成灰影（玻璃）、纯黑（透明）或
         hover 变青色（桌面端实测）；官方 iconButton 默认就是透明底，主题
         只需守住「透明 + hover 不变底」两条即可。hover 文字/图标强制
         primary（官方 hover 反白在浅色玻璃底上不可见，实测设置按钮 hover
         时图标与文字消失——正是 hover 白字被浅底吞掉）。
         选择器覆盖两种侧边栏形态：悬浮侧边栏（web，按钮在 _root 内）与
         官方侧边栏（桌面端，按钮直接挂在 sidebarCol 下）——去掉中间
         _root 层以兼容桌面端 DOM 结构差异。 */
      [class$='_sidebarCol'] [class*='_root'] button[aria-haspopup],
      [class$='_sidebarCol'] [class*='_root'] button[class*='iconButton'],
      [class$='_sidebarCol'] button[aria-haspopup],
      [class$='_sidebarCol'] button[class*='iconButton'] {
        background: transparent !important;
      }
      /* 悬停高亮：中性底色（interactive-bg-hover，官方默认 hover 反馈），
         不随 info/accent token 反色（避免三点按钮 hover 变青的老问题）；
         文字保持 primary 保证浅/深底都可见。 */
      [class$='_sidebarCol'] [class*='_root'] button[aria-haspopup]:hover,
      [class$='_sidebarCol'] [class*='_root'] button[class*='iconButton']:hover,
      [class$='_sidebarCol'] button[aria-haspopup]:hover,
      [class$='_sidebarCol'] button[class*='iconButton']:hover {
        background: var(--dsw-alias-interactive-bg-hover) !important;
        color: var(--dsw-alias-label-primary) !important;
      }
      /* 弹窗（设置/工作台/记忆等，全部挂在侧边栏槽位里）打开时必须全屏
         居中：backdrop-filter 会把 fixed 弹窗的 containing block 锁进侧边栏
         宽度（玻璃模式的 280px），展开态 sidebarCol 的 overflow:hidden 也会
         裁剪它。门控覆盖官方 modal（role=dialog）与自绘窗口
         （data-dsh-sidebar-window）。弹窗期间只禁 blur——玻璃的半透明底色
         必须保留，否则弹窗打开时侧边栏会变成透明（露出花纹）；遮罩盖住
      ...
      /* 弹窗内部组件（下拉框/输入框等）的 _root 不得继承玻璃底色：玻璃的
         半透明背景只属于侧边栏本体。弹窗本体在 sidebarCol 内，直接给它的
         后代组件清背景，侧边栏本体的 root 不在弹窗内、不受影响。 */
      body:has([role='dialog'][aria-modal='true'], [data-dsh-sidebar-window]) [role='dialog'] [class*='_root'],
      body:has([role='dialog'][aria-modal='true'], [data-dsh-sidebar-window]) [data-dsh-sidebar-window] [class*='_root'] {
        background: transparent !important;
      }
      body:has([role='dialog'][aria-modal='true'], [data-dsh-sidebar-window]) [class$='_sidebarCol'] {
        overflow: visible !important;
      }
      /* Hero 页贴底（开关在 usage 插件设置里，经 body.theme-endfield-hero-dock
         门控）：标题 + 输入框真正钉在列底。
         justify-content:flex-end 推底 + overflow:hidden 禁止滚动——flex 的
         auto-margin 推底方案会把推送距离计入 scrollHeight，在 scrollBody 上
         凭空造出滚动条（实测溢出 322px）；flex-end 无此问题。composerHero
         居中的 32px 底部垫脚在贴底时收成 14px，输入框离底边留一点呼吸空隙。 */
      body.theme-endfield-hero-dock [data-phase='hero'] [data-conversation-scroll] {
        overflow: hidden !important;
        justify-content: flex-end !important;
      }
      body.theme-endfield-hero-dock [data-phase='hero'] [data-composer-seat] [class*='composerHero'] {
        padding-bottom: 14px !important;
      }
      /* 贴底时隐藏 hero 蓝色光晕：它绝对定位 + translateY(50%)，在贴底布局下
         向下溢出 322px 撑大滚动区（即使 overflow:hidden 也会被硬切出直线
         边缘）。光晕是居中 hero 的装饰，贴底时不要。 */
      body.theme-endfield-hero-dock [data-phase='hero'] [data-composer-seat] [class*='heroGlow'] {
      ...
      /* Radius contract for third-party surfaces (e.g. the usage widget): the
         theme ships square by default; round mode restores app-native 14px.
         Official (theme-less) pages never see these variables and keep their
         own radius via the var() fallback. */
      body {
        --edge-radius: 0;
      }
      body.theme-endfield-round {
        --edge-radius: 14px;
      }
      ...
      /* better-sidebar 的圆形图标按钮（齿轮/开关/标签操作）同为小圆形控件，
         直角规则必须放行，否则 hover 高亮从圆形变成方块。 */
      body:not(.theme-endfield-round) [class$='_cardGear'],
      body:not(.theme-endfield-round) [class$='_rowGear'],
      body:not(.theme-endfield-round) [class$='_toggleButton'],
      body:not(.theme-endfield-round) [class$='_tabClose'],
      body:not(.theme-endfield-round) [class$='_tabBarPlus'],
      body:not(.theme-endfield-round) [class$='_tabIcon'] {
        border-radius: 50% !important;
      }
      ...
      * {
      ...
      /* ---------- Workspace browser rows（稳定后缀匹配，跨构建）----------
         会话横条：默认强制透明（无背景，只有文字，透出面板玻璃）；
         hover/选中 = 半透明信号黄（主题色），亮暗模式统一。 */
      [class*="sessionRow"],
      [class*="projectRow"],
    ...
    const unmount = () => {
      ...
      /* The surface classes mean nothing without the stylesheet that styles
         them; drop them with it, same as the palette class above. */
      document.body.classList.remove(SURFACE_CLASS_GLASS, SURFACE_CLASS_SOLID)
      ...
      slots.inject('settings.section', () => {
        ...
        () => {
          ...
          const changeSurface = (next) => {
            if (typeof localStorage !== 'undefined') localStorage.setItem(SURFACE_KEY, next)
            setSurface(next)
            syncSurface()
          }
          ...
          const toggleSurfaceConv = () => {
            const next = !surfaceConv
            if (typeof localStorage !== 'undefined') localStorage.setItem(SURFACE_CONV_KEY, next ? '1' : '0')
            setSurfaceConv(next)
            syncSurface()
          }
          ...
          return R.createElement('div', { style: pageStyle }, [
            ...
            R.createElement('div', { key: 'surface', style: rowStyle },
              ...
              R.createElement('span', { style: { display: 'flex', gap: '8px', flex: '0 0 auto', alignItems: 'center' } },
                R.createElement('button', {
                  type: 'button',
                  onClick: () => changeSurface('glass'),
                  style: btnStyleFor(surface === 'glass'),
                }, '玻璃'),
                R.createElement('button', {
                  type: 'button',
                  onClick: () => changeSurface('transparent'),
                  style: btnStyleFor(surface === 'transparent'),
                }, '透明'),
                R.createElement('button', {
                  type: 'button',
                  onClick: () => changeSurface('solid'),
                  style: btnStyleFor(surface === 'solid'),
                }, '自定义'),
                surface === 'solid'
                  ? R.createElement('input', {
                      type: 'color',
                      value: surfaceColor,
                      onChange: (event) => { changeSurfaceColor(event.target.value) },
                      'aria-label': '侧边栏底色',
                      style: {
            ...
            /* 纯色表面「应用到对话区」子开关：只在自定义纯色模式下出现，
               勾选后对话区整体（含标题栏）用自定义色的半透明版本铺底。 */
            surface === 'solid'
              ? R.createElement('div', { key: 'surface-conv', style: { ...rowStyle, paddingLeft: '16px' } },
                R.createElement('span', { style: labelStyle },
                  '应用到对话区',
                  R.createElement('span', { style: hintStyle },
                    '对话区使用同一底色，但比侧边栏多一层半透明（花纹仍可透过）'
                  )
                ),
                R.createElement('button', {
                  type: 'button',
    ...
    ctx.effect(() => () => {
      ...
      if (contourBsbObserver) contourBsbObserver.disconnect()
      disposeSettings()
    })
  }
