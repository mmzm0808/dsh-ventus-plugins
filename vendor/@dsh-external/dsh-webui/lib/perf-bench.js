...
/** 视为瞬时的上游故障：限流与网关类 5xx（基准测试不应因一次抖动整体判死）。 */
const TRANSIENT_FAILURE = /\bHTTP (?:429|500|502|503|504)\b|fetch failed|network|ECONN|ETIMEDOUT|socket/i;
/**
 * 把上游非 2xx 响应转成人话：优先取 JSON 错误体里的 code/message（如
 * SERVICE_BUSY「API Key 鉴权服务暂时不可用」），避免裸 JSON 糊脸；
 * 限流/网关类附一句定性说明。
 */
function describeHttpFailure(status, text) {
    ...
    catch { /* 非 JSON body 保持原文截断 */ }
    const hint = status === 429 || status === 502 || status === 503 || status === 504
        ? '（上游供应商过载/限流，属临时故障）'
        : '';
...
/**
 * 给单次测量包上瞬时故障重试：429/网关 5xx 与网络抖动按 ~1s/2s 退避，
 * 至多 3 次尝试；仍失败则把「已自动重试」并入错误信息。
 */
async function measureWithRetry(attempt) {
    const maxRetries = 2;
    let retried = 0;
    for (;;) {
        try {
            ...
            const transient = TRANSIENT_FAILURE.test(message);
            if (!transient || retried >= maxRetries) {
                if (retried > 0)
                    throw new Error(message + '｜已自动重试 ' + retried + ' 次仍失败');
                throw error;
            }
            await delay(900 * 2 ** retried);
            retried++;
        }
    }
}
...
async function streamOnce(ctx, baseURL, apiKey, model, prompt, maxTokens, withUsageOption) {
    ...
    try {
        ...
        // 响应原文开头（诊断用）：整条流没有可解析增量时，把它附进错误信息。
        let rawHead = '';
        while (!doneReading) {
            ...
            for (const raw of lines) {
                ...
                // 增量字段形态因网关而异：标准 content、DeepSeek 风格 reasoning_content、OpenRouter 风格 reasoning。
        ...
        if (ttft === 0) {
            ...
            throw new Error('响应中没有任何流式增量（SSE data 帧）' + (head !== '' ? '｜响应开头: ' + head : ''));
        }
...
async function runBench(ctx, provider, model) {
    ...
    try {
        ...
        let withUsage = true;
        try {
            await measureWithRetry(() => streamOnce(ctx, baseURL, apiKey, model, SHORT_PROMPT, 8, true));
        }
        catch (error) {
        ...
        if (budgetLeft(state)) {
            ...
            try {
                for (let i = 0; i < 5 && budgetLeft(state); i++) {
                    const r = await measureWithRetry(() => streamOnce(ctx, baseURL, apiKey, model, SHORT_PROMPT, 48, withUsage));
        ...
        if (budgetLeft(state)) {
            ...
            try {
                for (let i = 0; i < 3 && budgetLeft(state); i++) {
                    const r = await measureWithRetry(() => streamOnce(ctx, baseURL, apiKey, model, SHORT_PROMPT, 256, withUsage));
        ...
        if (budgetLeft(state)) {
            ...
            try {
                ...
                for (let i = 0; i < 3 && budgetLeft(state); i++) {
                    const r = await measureWithRetry(() => streamOnce(ctx, baseURL, apiKey, model, LONG_PROMPT, 8, withUsage));
