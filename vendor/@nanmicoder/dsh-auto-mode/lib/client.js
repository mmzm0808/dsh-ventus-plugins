...
`;
		...
		function decorateAutoPermissionIcons(document) {
			for (const marked of document.querySelectorAll(`[${ICON_ATTRIBUTE}]`)) {
				const kind = marked.getAttribute(ICON_ATTRIBUTE);
				if (kind === "menu" && !isAutoMenuItem(marked) || kind === "trigger" && !isAutoTrigger(marked)) marked.removeAttribute(ICON_ATTRIBUTE);
			}
			for (const menu of document.querySelectorAll("[role=\"menu\"]")) {
				if (!isPermissionMenu(menu)) continue;
				for (const item of menu.querySelectorAll("button[role=\"menuitem\"]")) if (normalizedText(item) === "Auto") item.setAttribute(ICON_ATTRIBUTE, "menu");
			}
			for (const button of document.querySelectorAll("button[aria-label]")) if (isAutoTrigger(button)) button.setAttribute(ICON_ATTRIBUTE, "trigger");
		}
		/** Install the Auto icon and explicit risk gate, then return their disposer. */
		function installAutoPermissionIcon(document) {
