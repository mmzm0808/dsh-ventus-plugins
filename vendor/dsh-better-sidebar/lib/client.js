window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function revealTab(state, tabId, dirs) {
			if (dirs.length === 0) return state;
			const expanded = [...state.expanded];
			let grew = false;
			for (const dir of dirs) if (!expanded.includes(dir)) {
				expanded.push(dir);
				grew = true;
			}
			let next = {
			...
			const tab = (leafWithTab(next.splits, tabId) ?? leafWithTab(next.bottomSplits, tabId))?.tabs.find((candidate) => candidate.id === tabId);
			if (tab === void 0) return next;
			const meta = tab.meta !== null && typeof tab.meta === "object" && !Array.isArray(tab.meta) ? tab.meta : {};
			if (meta.treeOpen === true) return next;
			return patchTab(next, tabId, { meta: {
				...meta,
				treeOpen: true
			} });
		}
		...
		* {@link relativeTo}.
		*/
		function dirsToReveal(filePath, cwd) {
		...
		function createBetterSidebarService(store) {
			...
			const activateTab$1 = (tabId, scope) => {
				...
				if (activated !== void 0) {
					...
					const cwd = scope?.cwd;
					if (activated.path !== void 0 && cwd !== void 0) {
		...
		*/
		...
		const zh = {
			...
			infoTotalChars: "总字数 {n}",
			infoSelectedChars: "选中 {n}",
			openFileNewTab: "用系统默认程序打开",
			openFileSide: "分屏打开",
			newTab: "新建标签页",
			openExplorer: "资源管理器",
		...
		const en = {
			...
			infoTotalChars: "Total chars {n}",
			infoSelectedChars: "Selected {n}",
			openFileNewTab: "Open with System Default App",
			openFileSide: "Open Split",
			newTab: "New tab",
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:E:\ProgramData\deepseek-harness\DSH_Anything\dsh-better-sidebar\src\client\sidebar.module.css.mjs
		const css$3 = "[data-dsh-panel-host]{z-index:5;pointer-events:none;position:fixed;inset:0}[data-dsh-panel-host][data-dsh-panel-host-degraded]{position:absolute;top:0;left:0}.yL8ocG_toggleCluster{top:calc(3px + env(safe-area-inset-top));z-index:8;pointer-events:auto;flex-direction:row;gap:4px;display:flex;position:absolute;right:10px}.yL8ocG_panel:not(.yL8ocG_panelHidden) .yL8ocG_tabBar{padding-right:72px}.yL8ocG_toggleButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), color var(--ds-transition-duration-slow) var(--ds-ease-in-out);background:0 0;border:none;border-radius:50%;justify-content:center;align-items:center;display:flex}.yL8ocG_toggleButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_toggleButton:disabled{opacity:.4;cursor:default}.yL8ocG_panel{z-index:5;pointer-events:auto;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;top:0;bottom:0;right:0}.yL8ocG_panelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), width var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translate(102%)}.yL8ocG_panel[data-dragging]{transition:none}.yL8ocG_panelResize{cursor:col-resize;z-index:2;touch-action:none;width:8px;position:absolute;top:0;bottom:0;left:-4px}.yL8ocG_panelResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.yL8ocG_panelBody{flex:1;min-width:0;min-height:0;display:flex}.yL8ocG_bottomPanel{z-index:5;background:var(--dsw-alias-bg-layer-1);border-top:1px solid var(--dsw-alias-border-l2);pointer-events:auto;padding-bottom:env(safe-area-inset-bottom);transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out);flex-direction:column;display:flex;position:absolute;bottom:0}.yL8ocG_bottomPanelHidden{pointer-events:none;visibility:hidden;transition:transform var(--ds-transition-duration-slow) var(--ds-ease-in-out), height var(--ds-transition-duration-slow) var(--ds-ease-in-out), visibility 0s linear var(--ds-transition-duration-slow);transform:translateY(102%)}.yL8ocG_bottomPanel[data-dragging]{transition:none}.yL8ocG_panel,.yL8ocG_bottomPanel{contain:layout style}body[data-dsh-sidebar-dragging] .yL8ocG_panel,body[data-dsh-sidebar-dragging] .yL8ocG_bottomPanel{will-change:transform}.yL8ocG_bottomResize{cursor:row-resize;z-index:2;touch-action:none;height:8px;position:absolute;top:-4px;left:0;right:0}.yL8ocG_bottomResizeActive{background:var(--dsw-alias-interactive-bg-hover-accent)}.yL8ocG_bottomClose{z-index:4;width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:3px;right:6px}.yL8ocG_bottomClose:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_bottomPanel .yL8ocG_tabBar{padding-right:40px}body[data-dsh-title-bar-compat] .yL8ocG_toggleCluster{top:calc(var(--dsh-title-bar-strip,40px) + 3px)}body[data-dsh-title-bar-compat] .yL8ocG_panel{padding-top:var(--dsh-title-bar-strip,40px)}.yL8ocG_cornerHandle{left:-6px;bottom:calc(var(--dsh-sidebar-height,0px) + 6px);z-index:2;cursor:nwse-resize;touch-action:none;width:12px;height:12px;position:absolute}.yL8ocG_cornerHandle:hover,.yL8ocG_cornerHandle[data-dragging]{background:var(--dsw-alias-interactive-bg-hover-accent)}.yL8ocG_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.yL8ocG_iconButton:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_iconButton:disabled{opacity:.4;cursor:default}.yL8ocG_workbench,.yL8ocG_split{flex:1;min-width:0;min-height:0;display:flex}.yL8ocG_splitRow{flex-direction:row}.yL8ocG_splitCol{flex-direction:column}.yL8ocG_splitChild{display:flex;position:relative;overflow:hidden}.yL8ocG_divider{z-index:3;touch-action:none;flex:none;position:relative}.yL8ocG_dividerRow:after,.yL8ocG_dividerCol:after{content:\"\";background:var(--dsw-alias-border-l2);transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);position:absolute}.yL8ocG_dividerRow{cursor:col-resize;width:7px;margin:0 -2px}.yL8ocG_dividerRow:after{width:1px;top:0;bottom:0;left:50%;transform:translate(-50%)}.yL8ocG_dividerCol{cursor:row-resize;height:7px;margin:-2px 0}.yL8ocG_dividerCol:after{height:1px;top:50%;left:0;right:0;transform:translateY(-50%)}.yL8ocG_divider:hover:after,.yL8ocG_dividerActive:after{background:var(--dsw-alias-interactive-bg-hover-accent)}.yL8ocG_pane{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;position:relative}.yL8ocG_paneDrop{outline:1px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.yL8ocG_dropOverlay{z-index:6;pointer-events:none;background:var(--dsw-alias-interactive-bg-hover-accent);opacity:.5;position:absolute}.yL8ocG_dropLeft{width:25%;top:0;bottom:0;left:0}.yL8ocG_dropRight{width:25%;top:0;bottom:0;right:0}.yL8ocG_dropUp{height:25%;top:0;left:0;right:0}.yL8ocG_dropDown{height:25%;bottom:0;left:0;right:0}.yL8ocG_dropCenter{outline:2px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-2px;background:0 0;inset:25%}.yL8ocG_paneContent{flex-direction:column;flex:1;min-height:0;display:flex;overflow:hidden}.yL8ocG_paneTab{flex-direction:column;flex:1;min-height:0;display:flex}.yL8ocG_paneTabHidden{display:none}.yL8ocG_paneEmptyCards{flex:1;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));align-content:start;gap:8px;min-height:0;padding:12px;display:grid;overflow:hidden}.yL8ocG_paneCard{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;text-align:center;border-radius:8px;flex-direction:column;justify-content:center;align-items:center;gap:6px;padding:12px 8px;display:flex}.yL8ocG_paneCard:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l2)}.yL8ocG_paneCard:disabled{opacity:.45;cursor:default}.yL8ocG_tabBar{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);flex:none;align-items:stretch;height:34px;display:flex}.yL8ocG_tabBarDrop{outline:1px dashed var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}.yL8ocG_tabList{scrollbar-width:none;flex:1;min-width:0;display:flex;overflow-x:auto}.yL8ocG_tabList::-webkit-scrollbar{display:none}.yL8ocG_tab{min-width:64px;max-width:160px;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);border-right:1px solid var(--dsw-alias-border-l1);cursor:pointer;user-select:none;background:0 0;flex:none;align-items:center;gap:4px;padding:0 4px 0 10px;display:flex}.yL8ocG_tab:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_tabActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.yL8ocG_tabTitle{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.yL8ocG_tabBadge{min-width:16px;height:15px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-brand-primary);border-radius:8px;flex:none;justify-content:center;align-items:center;padding:0 4px;display:inline-flex}.yL8ocG_tabClose{width:18px;height:18px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.yL8ocG_tabClose:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_tabBarPlus{background:var(--dsw-alias-bg-layer-1);width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;border:none;border-radius:5px;flex:none;justify-content:center;align-self:center;align-items:center;margin:0 6px;padding:0;display:inline-flex;position:sticky;right:0}.yL8ocG_tabBarPlus:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_explorer{flex-direction:column;flex:1;min-height:0;display:flex}.yL8ocG_explorerHeader{flex:none;justify-content:space-between;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.yL8ocG_explorerRoot{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.yL8ocG_explorerBody{flex:1;min-height:0;padding:2px 6px 8px;overflow:hidden auto}.yL8ocG_explorerRow{box-sizing:border-box;width:100%;max-width:100%;height:34px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;white-space:nowrap;animation:yL8ocG_dsh-row-in .15s var(--ds-ease-in-out);background:0 0;border:none;border-radius:8px;align-items:center;gap:6px;padding:0 8px;display:flex}.yL8ocG_explorerRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_explorerSelected{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.yL8ocG_explorerDir{font:var(--dsw-font-s-strong-14)}.yL8ocG_explorerHidden{opacity:.45}.yL8ocG_explorerSymlink{color:var(--dsw-alias-label-tertiary);flex:none}.yL8ocG_explorerBroken .yL8ocG_explorerName{color:var(--dsw-alias-state-error-primary)}.yL8ocG_explorerName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}.yL8ocG_explorerRef{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:20px;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;align-items:center;padding:0 8px;display:none}.yL8ocG_explorerRef:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_explorerRow:hover .yL8ocG_explorerRef,.yL8ocG_explorerRow:focus-within .yL8ocG_explorerRef{display:inline-flex}.yL8ocG_explorerCopied{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.yL8ocG_explorerError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);cursor:default}@keyframes yL8ocG_dsh-row-in{0%{opacity:0}}.yL8ocG_explorerEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.yL8ocG_editor{flex-direction:column;flex:1;min-height:0;display:flex}.yL8ocG_editorHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:6px;padding:4px 8px;display:flex}.yL8ocG_editorTitle{min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.yL8ocG_editorPathInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.yL8ocG_editorPathInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.yL8ocG_editorTreeToggleActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-active)}.yL8ocG_editorBody{flex:1;min-height:0;display:flex}.yL8ocG_editorMain{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.yL8ocG_editorTreeDock{border-left:1px solid var(--dsw-alias-border-l1);flex:none;min-height:0;display:flex;position:relative}.yL8ocG_editorTreeResize{cursor:col-resize;touch-action:none;z-index:3;width:6px;position:absolute;top:0;bottom:0;left:0}.yL8ocG_editorTreeResize:hover{background:var(--dsw-alias-border-l2)}.yL8ocG_editorTreePanel{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex}.yL8ocG_editorTreePanelFull{flex:1}.yL8ocG_editorTreeSearch{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.yL8ocG_editorSearchInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.yL8ocG_editorSearchInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.yL8ocG_editorSearchHint{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:8px 12px}.yL8ocG_editorSearchResult{width:100%;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);text-align:left;cursor:pointer;text-overflow:ellipsis;white-space:nowrap;background:0 0;border:none;border-radius:6px;padding:4px 8px;display:block;overflow:hidden}.yL8ocG_editorSearchResult:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_editorStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.yL8ocG_editorStatusError{color:var(--dsw-alias-state-error-primary)}.yL8ocG_dirtyDot{background:var(--dsw-alias-state-warn-primary);border-radius:50%;flex:none;width:7px;height:7px}.yL8ocG_editorPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex:1;justify-content:center;align-items:center;padding:16px;display:flex}.yL8ocG_orphanedType{opacity:.7;overflow-wrap:anywhere;margin-top:8px;font-size:12px;display:block}.yL8ocG_editorBinary{text-align:center;flex-direction:column;flex:1;justify-content:center;align-items:center;gap:12px;padding:24px 16px;display:flex}.yL8ocG_editorBinaryNotice{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.yL8ocG_editorDownloadLink{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-strong-12);cursor:pointer;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out), border-color var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:6px;align-items:center;gap:6px;padding:6px 14px;text-decoration:none;display:inline-flex}.yL8ocG_editorDownloadLink:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-border-l2)}.yL8ocG_editorError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);padding:12px 16px}.yL8ocG_editorBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.yL8ocG_sandboxStatus{font:var(--dsw-font-xxxs-11);flex:none;align-items:center;gap:8px;padding:4px 10px;display:flex}.yL8ocG_sandboxStatusOn{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border-bottom:1px solid var(--dsw-alias-border-l1)}.yL8ocG_sandboxStatusOff{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 10%, transparent);border-bottom:1px solid color-mix(in srgb, var(--dsw-alias-state-error-primary) 45%, transparent)}.yL8ocG_sandboxDot{background:var(--dsw-alias-state-success-primary);border-radius:50%;flex:none;width:6px;height:6px}.yL8ocG_sandboxStatusOff .yL8ocG_sandboxDot{background:var(--dsw-alias-state-error-primary)}.yL8ocG_sandboxStatusText{text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;overflow:hidden}.yL8ocG_sandboxAction{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:6px;flex:none;padding:2px 8px}.yL8ocG_sandboxAction:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_editorHtml{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.yL8ocG_browser{flex-direction:column;flex:1;min-height:0;display:flex}.yL8ocG_browserBar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:4px;padding:6px 8px;display:flex}.yL8ocG_browserInput{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);min-width:0;height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 10px}.yL8ocG_browserInput:focus{border-color:var(--dsw-alias-border-l2);outline:none}.yL8ocG_browserMessage{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex:none;padding:4px 12px}.yL8ocG_browserFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.yL8ocG_browserStart{text-align:center;min-height:0;font:var(--dsw-font-xs-13);color:var(--dsw-alias-label-tertiary);flex:1;justify-content:center;align-items:center;padding:20px;display:flex}.yL8ocG_browserBlocked{text-align:center;min-height:0;color:var(--dsw-alias-state-warn-primary);flex-direction:column;flex:1;justify-content:center;align-items:center;gap:6px;padding:24px;display:flex}.yL8ocG_browserBlockedTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary)}.yL8ocG_browserBlockedDesc{max-width:280px;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary)}.yL8ocG_browserBlockedActions{gap:8px;margin-top:6px;display:flex}.yL8ocG_browserBlockedButton{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-11);cursor:pointer;border-radius:6px;padding:4px 12px}.yL8ocG_browserBlockedButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_editorCm{background:0 0;flex:1;min-height:0;overflow:hidden}.yL8ocG_editorInfoBar{border-top:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;white-space:nowrap;user-select:none;flex:none;align-items:center;gap:16px;padding:4px 12px;font-size:11px;line-height:16px;display:flex}.yL8ocG_editorCmHidden{display:none}.yL8ocG_editorCm .cm-editor{height:100%}.yL8ocG_editorCm .cm-editor.cm-focused{outline:none}.yL8ocG_editorModeToggle{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;flex:none;align-items:center;gap:2px;padding:2px;display:inline-flex}.yL8ocG_editorModeButton{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;padding:2px 8px}.yL8ocG_editorModeButton:hover{color:var(--dsw-alias-label-primary)}.yL8ocG_editorModeActive{background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.yL8ocG_editorImageWrap{flex:1;justify-content:center;align-items:center;min-height:0;padding:12px;display:flex;overflow:auto}.yL8ocG_editorImage{object-fit:contain;max-width:100%;max-height:100%}.yL8ocG_editorMd{min-height:0;font:var(--dsw-font-xs-13);flex:1;padding:10px 14px;overflow-y:auto}.yL8ocG_mermaidWrap{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-1);border-radius:6px;margin:6px 0;overflow:hidden}.yL8ocG_mermaidHeader{border-bottom:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);justify-content:space-between;align-items:center;gap:6px;padding:4px 8px;display:flex}.yL8ocG_mermaidInfo{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary)}.yL8ocG_mermaidCopy{height:20px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-11);cursor:pointer;background:0 0;border:none;border-radius:4px;align-items:center;gap:4px;padding:0 6px;display:inline-flex}.yL8ocG_mermaidCopy:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_mermaidBody{cursor:zoom-in;justify-content:center;padding:10px;display:flex;overflow:auto}.yL8ocG_mermaidBody svg{max-width:100%;height:auto}.yL8ocG_mermaidError{border-bottom:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-11);padding:6px 10px}.yL8ocG_mermaidCode{font:var(--dsw-font-xxxs-11);margin:0;padding:8px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;overflow:auto}.yL8ocG_mermaidMarkdown .md-code-block[data-mermaid-processed]{display:contents}.yL8ocG_mermaidModal{z-index:1000;background:var(--dsw-alias-bg-mask-1);backdrop-filter:blur(2px);flex-direction:column;justify-content:center;align-items:center;display:flex;position:fixed;inset:0}.yL8ocG_mermaidModalToolbar{z-index:5;gap:8px;display:flex;position:absolute;top:16px;right:16px}.yL8ocG_mermaidModalButton{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);width:36px;height:36px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xs-strong-13);cursor:pointer;border-radius:8px;justify-content:center;align-items:center;display:inline-flex}.yL8ocG_mermaidModalButton:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_mermaidModalStage{justify-content:center;align-items:center;width:90vw;height:80vh;display:flex;position:relative;overflow:hidden}.yL8ocG_mermaidModalStage svg{cursor:grab;transform-origin:50%;user-select:none;-webkit-user-drag:none;background:var(--dsw-alias-bg-layer-1);border-radius:12px;max-width:none;max-height:none;padding:16px}.yL8ocG_mermaidModalStage svg:active{cursor:grabbing}.yL8ocG_mermaidModalHint{color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xxxs-11);pointer-events:none;position:absolute;bottom:16px;left:50%;transform:translate(-50%)}.yL8ocG_selectionPopup{z-index:5;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-layer-2);height:28px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;cursor:pointer;border-radius:6px;align-items:center;padding:0 10px;display:inline-flex;position:fixed;transform:translate(-50%,calc(-100% - 8px))}.yL8ocG_selectionPopup:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_editorPdf{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex}.yL8ocG_editorPdfToolbar{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;justify-content:flex-end;padding:6px 8px;display:flex}.yL8ocG_editorPdfStage{flex:1;min-height:0;display:flex;position:relative}.yL8ocG_editorPdfFrame{background:var(--dsw-alias-bg-base);border:none;flex:1;width:100%;min-height:0}.yL8ocG_editorPdfFrameBlocked{pointer-events:none}.yL8ocG_editorPdfDragShield{z-index:4;pointer-events:none;background:0 0;position:absolute;inset:0}.yL8ocG_editorPdfDragShieldActive{pointer-events:auto}body[data-dsh-tab-dragging] .yL8ocG_editorPdfFrame{pointer-events:none!important}body[data-dsh-tab-dragging] .yL8ocG_editorPdfDragShield{pointer-events:auto!important}.yL8ocG_terminalWrap{background:var(--dsw-alias-bg-base);flex-direction:column;flex:1;min-height:0;display:flex;position:relative}.yL8ocG_terminal{flex:1;min-height:0;padding:6px 4px 6px 8px}.yL8ocG_terminal .xterm{height:100%}.yL8ocG_terminalBanner{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-wrap:wrap;flex:none;align-items:center;gap:8px;padding:3px 10px;display:flex}.yL8ocG_terminalBannerUrl{word-break:break-all;opacity:.85;flex-basis:100%;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.yL8ocG_boundaryError{z-index:5;background:var(--dsw-alias-bg-layer-1);border-left:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;align-items:flex-start;gap:8px;padding:16px;display:flex;position:fixed;top:0;bottom:0;right:0;overflow:auto}.yL8ocG_terminalRetry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;border-radius:999px;flex:none;padding:1px 8px}.yL8ocG_terminalRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_terminalDepsBanner{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-warn-label);background:var(--dsw-alias-state-warn-tertiary);flex-direction:column;flex:none;gap:6px;padding:10px;display:flex}.yL8ocG_terminalDepsTitle{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-state-warn-primary)}.yL8ocG_terminalDepsHint{opacity:.9}.yL8ocG_terminalDepsCommandRow{align-items:flex-start;gap:8px;display:flex}.yL8ocG_terminalRepairCommand{white-space:pre-wrap;word-break:break-all;user-select:text;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:4px;flex:1;max-height:160px;margin:0;padding:6px 8px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:1.5;overflow:auto}.yL8ocG_terminalDepsNote{opacity:.85}.yL8ocG_terminalDepsActions{align-items:center;gap:8px;display:flex}.yL8ocG_tabBoundaryError{min-height:0;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);flex-direction:column;flex:1;align-items:flex-start;gap:8px;padding:12px 16px;display:flex;overflow:auto}.yL8ocG_git{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.yL8ocG_gitHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.yL8ocG_gitBranchSelect{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);min-width:0;height:26px;color:var(--dsw-alias-label-primary);font:var(--dsw-font-xxs-12);border-radius:6px;flex:1;padding:0 6px}.yL8ocG_gitSection{border-top:1px solid var(--dsw-alias-border-l1)}.yL8ocG_gitSectionHeader{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);text-transform:uppercase;justify-content:space-between;align-items:center;padding:6px 12px 4px;display:flex}.yL8ocG_gitLink{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-brand-primary);cursor:pointer;background:0 0;border:none;padding:0}.yL8ocG_gitLink:hover:not(:disabled){text-decoration:underline}.yL8ocG_gitLink:disabled{opacity:.4;cursor:default}.yL8ocG_gitRow{min-height:34px;animation:yL8ocG_dsh-row-in .15s var(--ds-ease-in-out);border-radius:8px;align-items:center;gap:6px;margin:0 6px;padding:0 8px;display:flex}.yL8ocG_gitRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_gitRowSelected{background:var(--dsw-alias-interactive-bg-active)}.yL8ocG_gitRowMain{cursor:pointer;text-align:left;background:0 0;border:none;flex:1;align-items:center;gap:8px;min-width:0;padding:3px 0;display:flex}.yL8ocG_gitBadge{width:20px;height:16px;font:var(--dsw-font-xxxs-strong-11);background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;justify-content:center;align-items:center;display:inline-flex}.yL8ocG_gitName{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.yL8ocG_gitEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);padding:4px 12px 8px}.yL8ocG_gitPlaceholder{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;padding:16px}.yL8ocG_gitError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);white-space:pre-wrap;padding:8px 12px}.yL8ocG_gitDiff{border-top:1px solid var(--dsw-alias-border-l1);padding:8px}.yL8ocG_gitDiffTab{flex-direction:column;flex:1;min-width:0;min-height:0;display:flex;overflow:hidden auto}.yL8ocG_gitDiffTabHeader{border-bottom:1px solid var(--dsw-alias-border-l1);flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.yL8ocG_gitDiffTabTitle{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.yL8ocG_gitDiffFile{align-items:baseline;gap:6px;padding:8px 2px 2px;display:flex}.yL8ocG_gitDiffFilePath{font:var(--dsw-font-xxs-strong-12);color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.yL8ocG_gitDiffFileOld{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;flex:none;max-width:40%;overflow:hidden}.yL8ocG_gitDiffFileTag{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:0 6px}.yL8ocG_gitDiffHunk{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);gap:8px;padding:3px 2px;display:flex}.yL8ocG_gitDiffHunkHeader{color:var(--dsw-alias-label-secondary);flex:none}.yL8ocG_gitDiffHunkSection{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.yL8ocG_gitDiffLine{font:var(--dsw-font-markdown-code-block-small);white-space:pre-wrap;overflow-wrap:anywhere;align-items:stretch;min-width:0;line-height:20px;display:flex}.yL8ocG_gitDiffNum{text-align:right;width:36px;color:var(--dsw-alias-label-tertiary);user-select:none;flex:none;padding-right:8px}.yL8ocG_gitDiffCode{flex:1;min-width:0;overflow:visible}.yL8ocG_gitDiffCtx{color:var(--dsw-alias-label-primary)}.yL8ocG_gitDiffDel{color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent)}.yL8ocG_gitDiffAdd{color:var(--dsw-alias-state-success-primary);background:color-mix(in srgb, var(--dsw-alias-state-success-primary) 12%, transparent)}.yL8ocG_gitDiffMeta{padding-left:2px}.yL8ocG_gitDiffMetaText{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);font-style:italic}.yL8ocG_gitDiffExpand{width:100%;font:var(--dsw-font-xxs-12);color:var(--dsw-alias-brand-primary);cursor:pointer;text-align:center;background:0 0;border:none;margin:4px 0;display:block}.yL8ocG_gitDiffExpand:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_gitConfirmDesc{font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);white-space:pre-wrap;margin:0}.yL8ocG_gitCommit{border-top:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;padding:8px 12px;display:flex}.yL8ocG_gitCommitInput{flex:1;min-width:0}.yL8ocG_gitCommitButton{background:var(--dsw-alias-button-primary-fill);height:26px;color:var(--dsw-alias-label-primary-inverted);font:var(--dsw-font-xxs-strong-12);cursor:pointer;border:none;border-radius:6px;flex:none;padding:0 12px}.yL8ocG_gitCommitButton:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}.yL8ocG_gitCommitButton:disabled{opacity:.45;cursor:default}.yL8ocG_gitLogRow{cursor:pointer;border-radius:8px;flex-direction:column;gap:2px;padding:5px 12px;display:flex}.yL8ocG_gitLogRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.yL8ocG_gitLogLine1{align-items:baseline;gap:8px;min-width:0;display:flex}.yL8ocG_gitLogHash{font:var(--dsw-font-markdown-code-block-small);color:var(--dsw-alias-label-tertiary);flex:none}.yL8ocG_gitLogLine2{flex-wrap:wrap;align-items:center;gap:6px;min-width:0;display:flex}.yL8ocG_gitLogRef{border:1px solid var(--dsw-alias-border-l2);font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-brand-primary);white-space:nowrap;border-radius:999px;flex:none;padding:0 5px}.yL8ocG_gitLogSubject{text-overflow:ellipsis;white-space:nowrap;min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.yL8ocG_gitLogMeta{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.yL8ocG_gitLogMore{border:1px solid var(--dsw-alias-border-l2);width:calc(100% - 24px);font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:6px;margin:4px 12px 8px;padding:6px 0;display:block}.yL8ocG_gitLogMore:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_gitLogMore:disabled{opacity:.5;cursor:default}.yL8ocG_producedRow{flex-wrap:wrap;align-items:center;gap:8px;padding:4px 0;display:flex}.yL8ocG_producedLabel{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.yL8ocG_producedChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);max-width:200px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxs-12);cursor:pointer;border-radius:999px;align-items:center;gap:4px;padding:2px 8px;display:inline-flex;overflow:hidden}.yL8ocG_producedChip:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.yL8ocG_producedChip span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.yL8ocG_producedMore{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary)}.yL8ocG_toggleButton:focus-visible,.yL8ocG_bottomClose:focus-visible,.yL8ocG_iconButton:focus-visible,.yL8ocG_tab:focus-visible,.yL8ocG_tabClose:focus-visible,.yL8ocG_tabBarPlus:focus-visible,.yL8ocG_paneCard:focus-visible,.yL8ocG_explorerRow:focus-visible,.yL8ocG_explorerRef:focus-visible,.yL8ocG_gitRowMain:focus-visible,.yL8ocG_gitLink:focus-visible,.yL8ocG_gitCommitButton:focus-visible,.yL8ocG_gitLogRow:focus-visible,.yL8ocG_gitLogMore:focus-visible,.yL8ocG_gitDiffExpand:focus-visible,.yL8ocG_terminalRetry:focus-visible,.yL8ocG_editorModeButton:focus-visible,.yL8ocG_editorDownloadLink:focus-visible,.yL8ocG_editorPptxButton:focus-visible,.yL8ocG_editorDocxZoomRange:focus-visible{outline:2px solid var(--dsw-alias-interactive-bg-hover-accent);outline-offset:-1px}@media (prefers-reduced-motion:reduce){.yL8ocG_panel,.yL8ocG_panelHidden,.yL8ocG_bottomPanel,.yL8ocG_bottomPanelHidden,.yL8ocG_toggleCluster,.yL8ocG_toggleButton,.yL8ocG_tab,.yL8ocG_tabBarPlus,.yL8ocG_paneCard,.yL8ocG_explorerRow,.yL8ocG_gitRow,.yL8ocG_divider,.yL8ocG_dividerRow:after,.yL8ocG_dividerCol:after{transition:none;animation:none}}@media (width<=767px){.yL8ocG_panel:not(.yL8ocG_panelHidden) .yL8ocG_tabBar{padding-right:40px}.yL8ocG_tab{min-width:48px;max-width:128px}}";
		...
		*/
		...
		function FileTree(props) {
			...
			*  exist once their ancestor levels have loaded). */
			...
			const selected = selectedPath === void 0 ? void 0 : toPosix(selectedPath);
			...
			const renderLevel = (dir, depth) => {
				...
				return (level.entries ?? []).map((entry) => {
					...
					if (entry.isDir) {
						...
						}), isOpen && renderLevel(posixPath, depth + 1)] }, entry.path);
					}
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						ref: (el) => {
							if (el === null) rowRefs.current.delete(posixPath);
							else rowRefs.current.set(posixPath, el);
						},
						role: "button",
						tabIndex: 0,
						className: clsx(sidebar_module_css_default.explorerRow, entry.hidden && sidebar_module_css_default.explorerHidden, entry.broken && sidebar_module_css_default.explorerBroken, selected === posixPath && sidebar_module_css_default.explorerSelected),
						style: { paddingLeft: depth * 22 + 6 },
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				}), root !== void 0 && data[toPosix(root)] !== void 0 && renderLevel(root, 1)] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
					open: rowMenu !== null,
					onClose: () => {
						setRowMenu(null);
					},
		...
		*/
		function TreePanel(props) {
			const { sessionId, cwd, expanded, onToggle, onOpenFile, onOpenFileNewTab, onOpenFileSide, onReferenceFile, full, revealPath, revealSeq } = props;
		...
		*  open and file tabs default closed. */
		...
		function EditorHost(props) {
			const { ctx, store, scope, tab, expanded, onToggleDir, onReferenceFile, revealPath, revealSeq } = props;
			const path = tab.path ?? "";
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						!showEmpty && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: sidebar_module_css_default.iconButton,
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:E:\ProgramData\deepseek-harness\DSH_Anything\dsh-better-sidebar\src\client\SubagentView.module.css.mjs
		const css$2 = ".pMEqhG_subagent{flex-direction:column;flex:1;min-height:0;display:flex}.pMEqhG_subagentHeader{flex:none;align-items:center;gap:8px;height:36px;padding:0 8px 0 12px;display:flex}.pMEqhG_subagentTitle{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.pMEqhG_subagentCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.pMEqhG_subagentRefresh{width:24px;height:24px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;display:inline-flex}.pMEqhG_subagentRefresh:hover{background:var(--dsw-alias-interactive-bg-hover)}.pMEqhG_subagentBody{flex:1;min-height:0;padding:2px 6px 8px;overflow-y:auto}.pMEqhG_subagentRow{box-sizing:border-box;width:100%;min-height:50px;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;align-items:flex-start;gap:8px;padding:7px 8px 7px 11px;display:flex;position:relative}.pMEqhG_subagentRow:hover,.pMEqhG_subagentRow:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.pMEqhG_subagentRowActive,.pMEqhG_subagentRowActive:hover,.pMEqhG_subagentRowActive:focus-visible{background:var(--dsw-alias-interactive-bg-active)}.pMEqhG_subagentRowDisabled{color:var(--dsw-alias-label-dimmed);cursor:not-allowed}.pMEqhG_subagentRowDisabled:hover{background:0 0}.pMEqhG_subagentRowLoading{cursor:default}.pMEqhG_subagentDot{margin-top:4px}.pMEqhG_subagentContent{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.pMEqhG_subagentLabel,.pMEqhG_subagentSecondary{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.pMEqhG_subagentLabel{color:inherit;font-weight:400}.pMEqhG_subagentSecondary{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary)}.pMEqhG_subagentLive{min-width:0;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);align-items:baseline;gap:4px;display:flex;overflow:hidden}.pMEqhG_subagentLiveTool{font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);flex:none}.pMEqhG_subagentLiveArgs{min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.pMEqhG_subagentLiveText{-webkit-line-clamp:2;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-secondary);-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.pMEqhG_subagentNode{min-width:0;position:relative}.pMEqhG_subagentChildren{margin-left:18px;padding-left:4px;position:relative}.pMEqhG_subagentChildren:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);height:26px;position:absolute;top:-26px;left:0}.pMEqhG_subagentChildren[aria-busy=true]:before{content:none}.pMEqhG_subagentChildren>.pMEqhG_subagentNode:before{content:\"\";border-left:1px solid var(--dsw-alias-border-l2);position:absolute;top:0;bottom:0;left:-4px}.pMEqhG_subagentChildren>.pMEqhG_subagentNode:last-child:before{height:17px;bottom:auto}.pMEqhG_subagentChildren>.pMEqhG_subagentNode>.pMEqhG_subagentRow:before{content:\"\";border-top:1px solid var(--dsw-alias-border-l2);width:14px;position:absolute;top:16px;left:-4px}.pMEqhG_subagentEmpty{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-label-tertiary);text-align:center;flex-direction:column;gap:2px;padding:16px;display:flex}.pMEqhG_subagentEmptyHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-dimmed)}.pMEqhG_subagentError{font:var(--dsw-font-xxs-12);color:var(--dsw-alias-state-error-primary);justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;display:flex}.pMEqhG_subagentErrorRetry{height:24px;color:var(--dsw-alias-label-secondary);font:var(--dsw-font-xxxs-strong-11);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;align-items:center;gap:4px;padding:0 8px;display:inline-flex}.pMEqhG_subagentErrorRetry:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-primary)}.pMEqhG_jobs{border-top:1px solid var(--dsw-alias-border-l2);margin-top:10px;padding-top:8px}.pMEqhG_jobsHeader{align-items:center;gap:8px;height:26px;padding:0 2px;display:flex}.pMEqhG_jobsTitle{min-width:0;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;flex:1;overflow:hidden}.pMEqhG_jobsCount{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.pMEqhG_jobsList{flex-direction:column;gap:2px;margin:0;padding:0;list-style:none;display:flex}.pMEqhG_jobsRow{border-radius:8px;align-items:center;gap:4px;display:flex}.pMEqhG_jobsRow:hover{background:var(--dsw-alias-interactive-bg-hover)}.pMEqhG_jobsRowSettled{opacity:.8}.pMEqhG_jobsRowSelected,.pMEqhG_jobsRowSelected:hover{background:var(--dsw-alias-interactive-bg-active)}.pMEqhG_jobsRowMain{min-width:0;font:var(--dsw-font-s-14);color:var(--dsw-alias-label-primary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:8px;outline:none;flex:1;align-items:flex-start;gap:8px;padding:6px 8px 6px 11px;display:flex}.pMEqhG_jobsRowMain:focus-visible{background:var(--dsw-alias-interactive-bg-hover)}.pMEqhG_jobsDot{margin-top:5px}.pMEqhG_jobsContent{flex-direction:column;gap:1px;min-width:0;display:flex}.pMEqhG_jobsLabelLine{align-items:center;gap:6px;min-width:0;display:flex}.pMEqhG_jobsKind{text-overflow:ellipsis;white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);max-width:90px;font:var(--dsw-font-xxxs-strong-11);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;padding:0 5px;line-height:14px;overflow:hidden}.pMEqhG_jobsLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.pMEqhG_jobsSecondary{text-overflow:ellipsis;white-space:nowrap;font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);overflow:hidden}.pMEqhG_jobsKill{width:22px;height:22px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:6px;flex:none;justify-content:center;align-items:center;margin-right:4px;display:inline-flex}.pMEqhG_jobsKill:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);color:var(--dsw-alias-state-error-primary)}.pMEqhG_jobsKillArmed,.pMEqhG_jobsKillArmed:hover{background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 12%, transparent);width:auto;height:20px;color:var(--dsw-alias-state-error-primary);font:var(--dsw-font-xxxs-strong-11);white-space:nowrap;padding:0 8px}.pMEqhG_jobsKill:disabled{opacity:.5;cursor:default}.pMEqhG_jobsKillError{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-state-error-primary);flex:none;margin-right:4px}.pMEqhG_jobsPane{z-index:1;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;margin-top:4px;position:sticky;bottom:0;overflow:hidden;box-shadow:0 -6px 12px -8px #00000059}.pMEqhG_jobsPaneHeader{border-bottom:1px solid var(--dsw-alias-border-l1);align-items:center;gap:6px;height:28px;padding:0 4px 0 10px;display:flex}.pMEqhG_jobsPaneDot{flex:none}.pMEqhG_jobsPaneLabel{text-overflow:ellipsis;white-space:nowrap;min-width:0;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);line-height:var(--dsw-font-xxxs-11-line-height);color:var(--dsw-alias-label-primary);flex:1;overflow:hidden}.pMEqhG_jobsPaneStatus{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);flex:none}.pMEqhG_jobsPaneClose{width:20px;height:20px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:5px;flex:none;justify-content:center;align-items:center;display:inline-flex}.pMEqhG_jobsPaneClose:hover{background:var(--dsw-alias-interactive-bg-hover)}.pMEqhG_jobsPanePre{max-height:200px;font-family:var(--ds-font-family-code);font-size:var(--dsw-font-xxxs-11-font-size);color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;margin:0;padding:6px 10px;line-height:1.5;overflow:auto}.pMEqhG_jobsPaneHint{font:var(--dsw-font-xxxs-11);color:var(--dsw-alias-label-tertiary);padding:8px 10px}.pMEqhG_jobsPaneError{color:var(--dsw-alias-state-error-primary)}";
			...
			return [
				{
					...
					component: ({ ctx, store, scope, tab, expanded, onToggleDir, onReferenceFile, revealPath, revealSeq }) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(EditorHost, {
						ctx,
						store,
						scope,
						tab,
		...
		* (mirror of the terminal view's own cap; the loop restarts on session switch). */
		...
		function TabContent(props) {
			...
			const revealPath = visible && tab.path !== void 0 && tab.path !== "" ? tab.path : void 0;
			(0, react.useEffect)(() => {
				if (revealPath === void 0 || cwd === void 0) return;
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:E:\ProgramData\deepseek-harness\DSH_Anything\dsh-better-sidebar\src\client\SideCardSection.module.css.mjs
		const css$1 = ".IBFzUa_section{flex-direction:column;gap:14px;width:100%;max-width:760px;display:flex}.IBFzUa_intro{color:var(--dsw-alias-label-tertiary);margin:0;padding:0 2px;font-size:13px;line-height:20px}.IBFzUa_group{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:16px;flex-direction:column;flex:none;gap:8px;padding:18px 20px 20px;display:flex}.IBFzUa_groupHeading{color:var(--dsw-alias-label-primary);align-items:baseline;gap:7px;padding:0 2px 6px;font-size:13px;font-weight:600;line-height:20px;display:flex}.IBFzUa_count{color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums;font-size:12px;font-weight:400;line-height:18px}.IBFzUa_grid{grid-template-columns:repeat(auto-fill,minmax(168px,1fr));gap:10px;display:grid}.IBFzUa_card{border:1px solid var(--dsw-alias-border-l2);font:inherit;color:inherit;cursor:pointer;background:0 0;border-radius:12px;flex-direction:column;transition:background .12s,border-color .12s;display:flex;position:relative}.IBFzUa_card:not(.IBFzUa_cardOn):hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-label-dimmed)}.IBFzUa_cardOn{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-interactive-bg-active)}.IBFzUa_cardMain{border-radius:inherit;width:100%;font:inherit;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;flex-direction:column;gap:6px;padding:12px;display:flex}.IBFzUa_cardMain:focus-visible,.IBFzUa_cardGear:focus-visible,.IBFzUa_rowGear:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.IBFzUa_cardTop{align-items:center;gap:8px;min-width:0;display:flex}.IBFzUa_cardIconChip{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);width:28px;height:28px;color:var(--dsw-alias-label-tertiary);border-radius:8px;flex:none;justify-content:center;align-items:center;display:inline-flex}.IBFzUa_cardOn .IBFzUa_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);background:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 12%, transparent);color:var(--dsw-alias-button-primary-fill)}.IBFzUa_cardTitle{min-width:0;color:var(--dsw-alias-label-secondary);white-space:nowrap;text-overflow:ellipsis;flex:1;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.IBFzUa_cardOn .IBFzUa_cardTitle{color:var(--dsw-alias-label-primary)}.IBFzUa_cardCheck{background:var(--dsw-alias-button-primary-fill);width:16px;height:16px;color:var(--dsw-alias-bg-layer-3);border-radius:50%;flex:none;justify-content:center;align-items:center;display:inline-flex}.IBFzUa_cardDesc{color:var(--dsw-alias-label-tertiary);white-space:nowrap;text-overflow:ellipsis;font-size:11px;line-height:16px;overflow:hidden}.IBFzUa_addCard{border-style:dashed;border-color:var(--dsw-alias-border-l2);text-align:left;align-items:flex-start;padding:12px}.IBFzUa_addCard:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.IBFzUa_addCard:hover .IBFzUa_cardTitle{color:var(--dsw-alias-label-primary)}.IBFzUa_addCard:hover .IBFzUa_cardIconChip{border-color:color-mix(in srgb, var(--dsw-alias-button-primary-fill) 35%, transparent);color:var(--dsw-alias-button-primary-fill)}.IBFzUa_addCard:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:2px}.IBFzUa_cardOn .IBFzUa_cardDesc{color:var(--dsw-alias-label-secondary)}.IBFzUa_cardWithGear .IBFzUa_cardDesc{padding-right:30px}.IBFzUa_cardGear{width:16px;height:16px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:50%;justify-content:center;align-items:center;padding:0;display:inline-flex;position:absolute;top:46px;right:12px}.IBFzUa_cardGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.IBFzUa_rowGear{width:22px;height:22px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:50%;flex:none;justify-content:center;align-items:center;padding:0;display:inline-flex}.IBFzUa_rowGear:hover{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}.IBFzUa_row{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;padding:12px 2px;display:flex}.IBFzUa_row:last-child{border-bottom:none}.IBFzUa_rowText{flex-direction:column;gap:4px;min-width:0;display:flex}.IBFzUa_title{color:var(--dsw-alias-label-primary);font-size:14px;line-height:22px}.IBFzUa_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;line-height:18px}.IBFzUa_switch{cursor:pointer;flex:none;display:inline-flex;position:relative}.IBFzUa_switchInput{opacity:0;width:1px;height:1px;margin:0;position:absolute}.IBFzUa_switchTrack{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:10px;align-items:center;width:36px;height:20px;padding:2px;transition:background .15s,border-color .15s;display:inline-flex}.IBFzUa_switchThumb{background:var(--dsw-alias-label-secondary);border-radius:50%;width:14px;height:14px;transition:transform .15s,background .15s;display:block}.IBFzUa_switch:hover .IBFzUa_switchTrack{border-color:var(--dsw-alias-label-dimmed)}.IBFzUa_switchInput:checked+.IBFzUa_switchTrack{border-color:var(--dsw-alias-button-primary-fill);background:var(--dsw-alias-button-primary-fill)}.IBFzUa_switchInput:checked+.IBFzUa_switchTrack .IBFzUa_switchThumb{background:var(--dsw-alias-bg-layer-3);transform:translate(16px)}.IBFzUa_switchInput:focus-visible+.IBFzUa_switchTrack{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.IBFzUa_control{flex:none;align-items:center;gap:6px;display:flex}.IBFzUa_percentInput{width:76px}.IBFzUa_typedInput{width:200px}.IBFzUa_typedInputNumber{width:76px}.IBFzUa_selectAnchor{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);max-width:220px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:8px;align-items:center;gap:6px;padding:4px 8px;font-size:13px;line-height:20px;display:flex}.IBFzUa_selectAnchor:hover{border-color:var(--dsw-alias-label-dimmed)}.IBFzUa_selectAnchorIcon{flex:none;display:inline-flex}.IBFzUa_selectAnchorText{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.IBFzUa_selectOption{align-items:center;gap:10px;min-width:200px;display:flex}.IBFzUa_selectOptionIcon{color:var(--dsw-alias-label-secondary);flex:none;display:inline-flex}.IBFzUa_selectOptionText{flex-direction:column;min-width:0;display:flex}.IBFzUa_suffix{color:var(--dsw-alias-label-secondary);font-size:14px;line-height:22px}.IBFzUa_popupDialog.IBFzUa_popupDialog{width:min(460px,100%)}.IBFzUa_popupRows{flex-direction:column;gap:10px;width:100%;display:flex}.IBFzUa_popupRow{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;justify-content:space-between;align-items:center;gap:16px;min-width:0;padding:14px 16px;transition:border-color .16s,background .16s;display:flex}.IBFzUa_popupRow:hover{border-color:var(--dsw-alias-label-dimmed)}.IBFzUa_done{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;padding:5px 14px;font-size:13px;line-height:1.5}.IBFzUa_done:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}.IBFzUa_error{color:var(--dsw-alias-state-error-primary);padding:10px 0 2px;font-size:12px;line-height:17px}.IBFzUa_pluginModal.IBFzUa_pluginModal{width:min(560px,100%)}.IBFzUa_pluginList{flex-direction:column;gap:12px;width:100%;display:flex}.IBFzUa_pluginTopicBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:pointer;border-radius:8px;padding:6px 12px;font-size:12px;line-height:18px}.IBFzUa_pluginTopicBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.IBFzUa_pluginTopicBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.IBFzUa_pluginEmpty{color:var(--dsw-alias-label-tertiary);padding:20px 2px;font-size:12px;line-height:18px}.IBFzUa_pluginEntries{flex-direction:column;gap:10px;display:flex}.IBFzUa_pluginEntry{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);border-radius:12px;flex-direction:column;gap:4px;padding:12px;display:flex}.IBFzUa_pluginEntryHead{justify-content:space-between;align-items:center;gap:12px;display:flex}.IBFzUa_pluginEntryActions{flex:none;align-items:center;gap:6px;display:inline-flex}.IBFzUa_pluginJumpBtn{appearance:none;border:1px solid var(--dsw-alias-border-l2);font:inherit;cursor:pointer;color:var(--dsw-alias-label-secondary);background:0 0;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.IBFzUa_pluginJumpBtn:hover{background:var(--dsw-alias-interactive-bg-hover);border-color:var(--dsw-alias-interactive-bg-hover-accent);color:var(--dsw-alias-label-primary)}.IBFzUa_pluginJumpBtn:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.IBFzUa_pluginName{appearance:none;min-width:0;font:inherit;color:var(--dsw-alias-label-primary);text-align:left;text-overflow:ellipsis;white-space:nowrap;cursor:pointer;background:0 0;border:0;padding:0;font-size:13px;font-weight:600;line-height:20px;text-decoration:none;overflow:hidden}.IBFzUa_pluginName:hover{color:var(--dsw-alias-button-primary-fill);text-decoration:underline}.IBFzUa_pluginDesc{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px}.IBFzUa_pluginInstall{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);white-space:nowrap;border-radius:8px;padding:6px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;line-height:16px;display:block;overflow-x:auto}.IBFzUa_pluginCopyBtn{appearance:none;font:inherit;cursor:pointer;background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3);border:1px solid #0000;border-radius:8px;flex:none;padding:3px 12px;font-size:12px;line-height:1.5}.IBFzUa_pluginCopyBtn:hover{background:var(--dsw-alias-button-primary-hover)}.IBFzUa_pluginCopyBtn:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:1px}@media (prefers-reduced-motion:reduce){.IBFzUa_card,.IBFzUa_switchTrack,.IBFzUa_switchThumb{transition:none}}.IBFzUa_versionBadge{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:999px;align-self:flex-start;align-items:center;gap:8px;padding:4px 12px 4px 14px;font-size:12px;line-height:18px;display:inline-flex}.IBFzUa_versionBadgeName{color:var(--dsw-alias-label-primary);font-weight:600}.IBFzUa_versionBadgeTag{background:var(--dsw-alias-accent-soft,var(--dsw-alias-border-l2));color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums;border-radius:999px;padding:1px 8px}.IBFzUa_pluginSearch{box-sizing:border-box;appearance:none;border:1px solid var(--dsw-alias-border-l2);width:100%;font:inherit;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);border-radius:8px;padding:6px 10px;font-size:12px;line-height:18px}.IBFzUa_pluginSearch::placeholder{color:var(--dsw-alias-label-tertiary)}.IBFzUa_pluginSearch:focus-visible{outline:2px solid var(--dsw-alias-border-l4);outline-offset:1px}.IBFzUa_pluginEntries{flex-direction:column;gap:10px;max-height:46vh;padding-right:2px;display:flex;overflow-y:auto}.IBFzUa_pluginGroup{flex-direction:column;gap:8px;display:flex}.IBFzUa_pluginGroupHeading{color:var(--dsw-alias-label-secondary);padding:2px 2px 0;font-size:12px;font-weight:600;line-height:18px}";
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:E:\ProgramData\deepseek-harness\DSH_Anything\dsh-better-sidebar\src\client\layout.css.mjs
			...
			try {
				...
				ctx.effect(() => {
					try {
						...
						window.__dbsOpenFileListener = true;
						const producedDisposer = registerProducedInterception({ openInEditor: (path) => {
							ctx.betterSidebar?.openTab({
								type: "editor",
								path,
								title: basenameOf(path)
						...
						return () => {
							...
							window.__dbsOpenFileListener = false;
							producedDisposer();
						};
					} catch (error) {
