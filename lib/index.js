...
/** 用户禁用的子插件 id 集合（更新弹窗取消勾选写入 disabled.json，重启生效）。 */
let disabledSubPlugins = new Set()
try {
  const raw = readFileSync(join(ROOT, 'disabled.json'), 'utf8')
  const arr = JSON.parse(raw)
  if (Array.isArray(arr)) disabledSubPlugins = new Set(arr)
} catch { /* 无配置文件 = 全部启用 */ }
...
const subConfigs = {
  ...
  // MinerU host 的 resolveConfig 要求 baseURL 非空，缺省会 throw 导致 RPC 不注册、设置页永远省略号。
  '@huanlin/dsh-plugin-mineru': { baseURL: 'http://127.0.0.1:18000' },
}
...
export function apply(ctx) {
  ...
  for (const [id, plugin] of Object.entries(subPlugins)) {
    if (disabledSubPlugins.has(id)) {
      console.warn(`[dsh-ventus-plugins] 子插件已被禁用（disabled.json），跳过: ${id}`)
      continue
    }
    try {
