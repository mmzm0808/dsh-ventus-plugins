...
import { gateConvention, gateNoVerify, transition, verdict, type ClaimStatus } from './gates.js'
...
test('transition 状态机主链', () => {
  const chain: Array<[ClaimStatus, Parameters<typeof transition>[1], ClaimStatus]> = [
  ...
  for (const [from, action, to] of chain) assert.equal(transition(from, action), to)
})
