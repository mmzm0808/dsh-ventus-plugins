...
- **独立 bundle 行**（`cordis.patch.yml` 的 `ventus-bench` 行），与整合包聚合行并列挂载。
...
```sh
# 从本仓库目录（整合包仓库内）安装
dsh plugin --profile web add "<本仓库绝对路径>/dsh-ventus-bench"
# 或本地开发
cd dsh-ventus-bench && DSH_CHECKOUT=<harness> bash scripts/build.sh
```

安装后**重启 DSH** 生效。验证：`dsh --profile web --dump-config` 应出现 `ventus-bench` 层。
