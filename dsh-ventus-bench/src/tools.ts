/**
 * dsh-ventus-bench — 科研工作流 7 工具（rb_open / rb_derive / rb_verify /
...
import { gateNoVerify, transition, verdict, type TolBand } from './gates.js'
import {
  createEmptyState,
  ensureTopicDirs,
  findClaim,
...
const DEFAULT_TOL: Record<string, TolBand> = {
...
function cwdOf(exec: ToolRunContext): string | null {
  return exec.agent?.session?.header?.cwd ?? null
}

/** 需要当前课题的工具的统一入口错误。 */
function needOpen(): { ok: false; error: string } {
  return { ok: false, error: '尚未打开课题，请先调用 rb_open { topic }' }
}

/** 读当前课题 state；未打开或文件损坏返回 null。 */
function loadCurrentState(): RbState | null {
  if (currentRoot === null) return null
  return readState(currentRoot)
}

/** 简易简报（设计稿：≤30 行三源重建 briefing）。 */
function buildBriefing(state: RbState, trust: 'high' | 'low', pending: ReturnType<typeof scanAssets>): string {
  const lines: string[] = []
  lines.push(`课题 ${state.topic}  briefing（root: ${state.root}，trust: ${trust}）`)
  if (state.conventions.length === 0) lines.push('· 口径声明：无（跨口径比较会被拒绝）')
  else lines.push(`· 口径声明：${state.conventions.map(c => c.id).join(', ')}`)
  if (state.claims.length === 0) lines.push('· claims：无')
  else {
  ...
  if (pending.length > 0) {
    ...
    for (const asset of pending) lines.push(`  - ${asset.path} (${asset.kind})`)
  }
  if (state.evidence.length > 0) lines.push(`· 证据：${state.evidence.length} 条（stance 待人工确认 ${state.evidence.filter(e => e.stance === 'pending').length}）`)
  if (state.pendingMemory.length > 0) lines.push(`· 待写入记忆 ${state.pendingMemory.length} 条（待人工确认）`)
  return lines.slice(0, 30).join('\n')
}

/** 解析 verify JSON 里的相对误差；支持 err/relative_error/result+expected/results 数组。 */
function parseVerifyError(payload: Record<string, unknown>): { err: number; detail: string } | { err: null; detail: string } {
  ...
  if (errDirect !== null) return { err: errDirect, detail: `直接误差 ${errDirect}` }

  const result = num(payload.result)
  const expected = num(payload.expected)
  if (result !== null && expected !== null) {
    if (expected === 0) return { err: Math.abs(result), detail: `|result|（expected=0）` }
  ...
  if (Array.isArray(payload.results) && Array.isArray(payload.expected) && payload.results.length === payload.expected.length) {
    ...
    for (let i = 0; i < payload.results.length; i++) {
      const r = num((payload.results as unknown[])[i])
      const e = num((payload.expected as unknown[])[i])
      if (r === null || e === null) return { err: null, detail: `results/expected 第 ${i} 项非数值` }
      if (e === 0) worst = Math.max(worst, Math.abs(r))
      else worst = Math.max(worst, Math.abs(r - e) / Math.abs(e))
    }
...
export function registerBenchTools(ctx: Context, env: BenchEnv): Array<() => void> {
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (cwd === null) return { ok: false, error: '无法确定会话工作区目录（exec.agent.session.header.cwd）' }
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (state === null) return needOpen()
      const claimId = args.claim_id.trim()
      if (claimId === '') return { ok: false, error: 'claim_id 不能为空' }
      ...
      if (claim === undefined) {
        if (args.text === undefined || args.text.trim() === '') return { ok: false, error: '新建 claim 必须提供 text' }
        claim = {
          id: claimId,
          version: 1,
          status: 'draft',
        ...
        if (claim.frozen) return { ok: false, error: `claim ${claimId} 已冻结（${claim.status}），需先解除冻结或新建版本` }
        claim.version += 1
        claim.text = args.text?.trim() ?? claim.text
        if (args.convention_id !== undefined) claim.conventionId = args.convention_id
        if (claim.status !== 'draft' && claim.status !== 'derived') claim.status = 'derived'
      }
      const next = transition(claim.status, 'derive')
      if (next !== null) claim.status = next
      // 推导修改后旧验证作废。
      if (claim.status !== 'derived') claim.status = 'derived'

      const deriveRef = join('notes', 'derivations', `${claimId}.md`)
      const file = join(currentRoot as string, deriveRef)
      mkdirSync(join(currentRoot as string, 'notes', 'derivations'), { recursive: true })
      const stamp = `# ${claimId} 推导\n\n- 更新时间: ${localIso()}\n- version: ${claim.version}\n- 口径: ${claim.conventionId ?? '（未声明）'}\n\n## 表达式\n\n${args.expression}\n\n## 声明文本\n\n${claim.text}\n`
      writeFileSync(file, stamp, 'utf8')
      claim.deriveRef = deriveRef

      const assetPath = join(currentRoot as string, deriveRef)
      state.assets.push({
      ...
      writeState(currentRoot as string, state)
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (claim === undefined) return { ok: false, error: `claim ${claimId} 不存在（先 rb_derive 创建）` }
      if (claim.frozen) return { ok: false, error: `claim ${claimId} 已冻结` }
      const tolClass = args.tol_class?.trim() ?? claim.tolClass ?? 'B'
      const band = tolClasses[tolClass]
      if (band === undefined) return { ok: false, error: `未知 tol_class ${tolClass}（可选 ${Object.keys(tolClasses).join('/')}）` }

      const root = currentRoot as string
      ...
      try {
        const parsed = JSON.parse(readFileSync(verifyFile, 'utf8')) as unknown
      ...
      if (parsed.err === null) return { ok: false, error: parsed.detail }

      const result = verdict(parsed.err, band)
      let nextStatus: string
      let action: 'verify-pass' | 'verify-warn' | 'verify-fail'
      if (result === 'PASS') { nextStatus = 'verified'; action = 'verify-pass' }
      else if (result === 'WARN') { nextStatus = 'needs-review'; action = 'verify-warn' }
      else { nextStatus = 'mismatch'; action = 'verify-fail' }

      const target = transition(claim.status, action)
      if (target === null) return { ok: false, error: `claim ${claimId} 当前状态 ${claim.status} 不允许验证（需要 derived/needs-review/mismatch）` }
      claim.status = target
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const claimId = args.claim_id.trim()
      const claim = findClaim(state, claimId)
      if (claim === undefined) return { ok: false, error: `claim ${claimId} 不存在` }
      if (claim.status !== 'verified') return { ok: false, error: `claim ${claimId} 状态为 ${claim.status}，需 verified 才能收集证据` }
      ...
      if (target === null) return { ok: false, error: `claim ${claimId} 状态 ${claim.status} 不允许收集证据` }
      claim.status = target
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const claim = findClaim(state, claimId)
      if (claim === undefined) return { ok: false, error: `claim ${claimId} 不存在` }
      if (claim.status !== 'evidenced') return { ok: false, error: `claim ${claimId} 状态为 ${claim.status}，需 evidenced 才能裁决` }
      ...
      if (target === null) return { ok: false, error: `claim ${claimId} 状态 ${claim.status} 不允许裁决` }
      claim.status = target
      claim.frozen = true
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      for (const rawId of args.claim_ids) {
        ...
        const claim = findClaim(state, claimId)
        if (claim === undefined) { blocked.push(`${claimId}（不存在）`); continue }
        const gate = gateNoVerify(claim)
        if (gate !== 'PASS') { blocked.push(`${claimId}（${gate}：${claim.status}）`); continue }
        usable.push(claim)
      }
      if (blocked.length > 0) {
        return { ok: false, error: `以下 claim 未通过硬闸门，拒绝写 tex：${blocked.join('、')}（先 rb_verify 且 rb_adjudicate）` }
      }
      if (usable.length === 0) return { ok: false, error: '没有可写入的 claim' }
      ...
      for (const claim of usable) {
        ...
        if (claim.verifyRef !== undefined) lines.push(`\\emph{已验证，误差档 ${claim.tolClass}（${claim.verifyRef}）。}`)
  ...
  disposers.push(ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const facts = args.facts
        ...
        .filter(f => f !== '')
      if (facts.length === 0) return { ok: false, error: 'facts 为空' }
      const at = localIso()
      for (const text of facts) state.pendingMemory.push({ text, projectScope: args.project_scope !== false, at })
...
export function setCurrentBenchRoot(root: string | null): void {
  currentRoot = root
}

/** 取当前课题根。 */
export function getCurrentBenchRoot(): string | null {
  return currentRoot
}
